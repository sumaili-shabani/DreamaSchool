<template>
  ...
  export default {
    ...
    methods: {
      ...
      testTitle() {
        if (this.edit == true) {
          this.titleComponent = "modification de " + item.taux;
        } else {
    ...
    //   }, 300),
      onPageChange() {
        this.fetch_data(`${this.apiBaseURL}/fetch_vente_taux?page=`);
      },
      ...
      validate() {
        ...
        if (this.$refs.form.validate()) {
          this.isLoading(true);          
          this.insertOrUpdate(
            `${this.apiBaseURL}/insert_vente_taux`,
      ...
      editData(id) {
        this.editOrFetch(`${this.apiBaseURL}/fetch_single_vente_taux/${id}`).then(
          ({ data }) => {
      ...
      clearP(id) {
        this.confirmMsg().then(({ res }) => {
          this.delGlobal(`${this.apiBaseURL}/delete_vente_taux/${id}`).then(
            ({ data }) => {
