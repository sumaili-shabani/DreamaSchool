                        ...
                        <v-form ref="form" lazy-validation>
                            ...
                            <v-card-text>
                                <v-layout row wrap>
                                    <v-flex xs12 sm12 md12 lg12>
                                        <div class="mr-1">
                                            <v-text-field label="Nom de la période" prepend-inner-icon="extension"
                                                :rules="[(v) => !!v || 'Ce champ est requis']" outlined dense
                                                v-model="svData.nomPeriode"></v-text-field>
                                        </div>
                                    </v-flex>
                                </v-layout>
                            </v-card-text>
                    ...
                    <table class="table">
                        <thead>
                            <tr>
                                ...
                                <th class="text-left">statutPeriode</th>
                        ...
                        <tbody>
                            <tr v-for="item in fetchData" :key="item.id">
                                <td>#{{ item.id }}</td>
                                <td>{{ item.nomPeriode }}</td>
                                <td>


                                    <v-badge center :color="item.statutPeriode == 1 ? 'success' : 'danger'"
                                        style="cursor: pointer">
                                        <span slot="badge">{{
                                            item.statutPeriode == 1 ? "Actif " : "Inactif"
                                            }}</span>
                                    </v-badge>
                                </td>
                                ...
                                <td>
                                    ...
                                    <a class="me-3" href="javascript:void(0);" @click="operationEtat(item.id, item.statutPeriode)">
                                        <img :src="`${baseURL}/vuetheme/assets/img/icons/time.svg`" alt="img" />
                                    </a>
...
export default {
    ...
    methods: {
        ...
        showModal() {
            this.dialog = true;
            this.titleComponent = "Ajout de la période";
            this.edit = false;
        ...
        testTitle() {
            if (this.edit == true) {
                this.titleComponent = "modification de " + item.nomPeriode;
            } else {
        ...
        onPageChange() {
            this.fetch_data(`${this.apiBaseURL}/fetch_periode?page=`);
        },
        ...
        validate() {
            if (this.$refs.form.validate()) {
                ...
                this.insertOrUpdate(
                    `${this.apiBaseURL}/insert_periode`,
        ...
        editData(id) {
            this.editOrFetch(
                `${this.apiBaseURL}/fetch_single_periode/${id}`
            ).then(({ data }) => {
        ...
        clearP(id) {
            this.confirmMsg().then(({ res }) => {
                this.delGlobal(`${this.apiBaseURL}/delete_periode/${id}`).then(
                    ({ data }) => {
        ...
        operationEtat(id, etat) {
            if (id != "" && etat != "") {
                ...
                this.confirmEtat().then(({ res }) => {
                    this.delGlobal(
                        `${this.apiBaseURL}/chect_etat_periode/${id}/${etat}`
                    ).then(({ data }) => {
                        this.showMsg(data.data);
                        this.onPageChange();
                    });
