                        ...
                        <v-form ref="form" lazy-validation>
                            ...
                            <v-card-text>
                                ...
                                <v-layout row wrap>
                                    <v-flex xs12 sm12 md12 lg12>
                                        <div class="mr-1">
                                            <v-autocomplete label="Selectionnez la catégorie de cours" prepend-inner-icon="home"
                                    ...
                                    <v-flex xs12 sm12 md12 lg12>
                                        <div class="mr-1">
                                            <v-text-field label="Nom du cours" prepend-inner-icon="extension"
                                                :rules="[(v) => !!v || 'Ce champ est requis']" outlined dense
                                                v-model="svData.nomCours"></v-text-field>
                                        </div>
                                    </v-flex>
                                </v-layout>
                            </v-card-text>
                    ...
                    <table class="table">
                        ...
                        <tbody>
                            <tr v-for="item in fetchData" :key="item.id">
                                <td>{{ item.nomCours | subStrLong2}}</td>
                                <td>{{ item.nomCatCours |subStrLong2}}</td>
...
export default {
    ...
    computed: {
        ...mapGetters(["roleList","CatCoursList", "isloading"]),
    },
    methods: {
        ...mapActions(["getRole", "getCatCoursList"]),

        showModal() {
            this.dialog = true;
            this.titleComponent = "Ajout de cours";
            this.edit = false;
        ...
        testTitle() {
            if (this.edit == true) {
                this.titleComponent = "modification de " + item.nomCours;
            } else {
        ...
        onPageChange() {
            this.fetch_data(`${this.apiBaseURL}/fetch_cours?page=`);
        },
        ...
        editData(id) {
            this.editOrFetch(
                `${this.apiBaseURL}/fetch_single_cours/${id}`
            ).then(({ data }) => {
        ...
        clearP(id) {
            this.confirmMsg().then(({ res }) => {
                this.delGlobal(`${this.apiBaseURL}/delete_cours/${id}`).then(
                    ({ data }) => {
