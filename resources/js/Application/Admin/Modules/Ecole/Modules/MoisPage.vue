                        ...
                        <v-form ref="form" lazy-validation>
                            ...
                            <v-card-text>
                                <v-layout row wrap>
                                    <v-flex xs12 sm12 md12 lg12>
                                        <div class="mr-1">
                                            <v-text-field label="Mois" prepend-inner-icon="extension"
                                                :rules="[(v) => !!v || 'Ce champ est requis']" outlined dense
                                                v-model="svData.nomMois"></v-text-field>
                                        </div>
                                    </v-flex>
                                </v-layout>
                            </v-card-text>
...
export default {
    ...
    methods: {
        ...
        showModal() {
            this.dialog = true;
            this.titleComponent = "Ajout de mois";
            this.edit = false;
        ...
        testTitle() {
            if (this.edit == true) {
                this.titleComponent = "modification de " + item.nomMois;
            } else {
                this.titleComponent = "Ajout de mois";
            }
        },
        ...
        onPageChange() {
            this.fetch_data(`${this.apiBaseURL}/fetch_mois_scolaire?page=`);
        },
        ...
        validate() {
            if (this.$refs.form.validate()) {
                ...
                this.insertOrUpdate(
                    `${this.apiBaseURL}/insert_mois_scolaire`,
        ...
        editData(id) {
            this.editOrFetch(
                `${this.apiBaseURL}/fetch_single_mois_scolaire/${id}`
            ).then(({ data }) => {
        ...
        clearP(id) {
            this.confirmMsg().then(({ res }) => {
                this.delGlobal(`${this.apiBaseURL}/delete_mois_scolaire/${id}`).then(
                    ({ data }) => {
