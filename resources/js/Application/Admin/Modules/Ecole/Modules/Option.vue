                        ...
                        <v-form ref="form" lazy-validation>
                            ...
                            <v-card-text>
                                <v-layout row wrap>
                                    ...
                                    <v-flex xs12 sm12 md12 lg12>
                                        <div class="mr-1">
                                            <v-autocomplete label="Selectionnez la section" prepend-inner-icon="home"
                                    ...
                                    <v-flex xs12 sm12 md12 lg12>
                                        <div class="mr-1">
                                            <v-text-field label="Nom de l'option" prepend-inner-icon="extension"
                                                :rules="[(v) => !!v || 'Ce champ est requis']" outlined dense
                                                v-model="svData.nomOption"></v-text-field>
                                        </div>
                                    </v-flex>
...
export default {
    ...
    computed: {
        ...mapGetters(["roleList","sectionList", "isloading"]),
    },
    methods: {
        ...mapActions(["getRole","getSection"]),

        showModal() {
            this.dialog = true;
            this.titleComponent = "Ajout de l'option";
            this.edit = false;
        ...
        testTitle() {
            if (this.edit == true) {
                this.titleComponent = "modification de " + item.nomOption;
            } else {
                this.titleComponent = "Ajout de l'option";
            }
        },
        ...
        onPageChange() {
            this.fetch_data(`${this.apiBaseURL}/fetch_option?page=`);
        },
        ...
        editData(id) {
            this.editOrFetch(
                `${this.apiBaseURL}/fetch_single_option/${id}`
            ).then(({ data }) => {
        ...
        clearP(id) {
            this.confirmMsg().then(({ res }) => {
                this.delGlobal(`${this.apiBaseURL}/delete_option/${id}`).then(
                    ({ data }) => {
