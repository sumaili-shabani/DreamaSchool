                                    ...
                                    <v-flex xs12 sm12 md12 lg12>
                                        <div class="mr-1">
                                            <v-text-field label="Désignation" prepend-inner-icon="extension"
                                                ...
                                                v-model="svData.nomSection"></v-text-field>
                                        </div>
                                    </v-flex>
                                </v-layout>
                            </v-card-text>
...
export default {
    ...
    methods: {
        ...
        showModal() {
            this.dialog = true;
            this.titleComponent = "Ajout de section";
            this.edit = false;
        ...
        testTitle() {
            if (this.edit == true) {
                this.titleComponent = "modification de " + item.nomSection;
            } else {
                this.titleComponent = "Ajout de section";
            }
        },
        ...
        onPageChange() {
            this.fetch_data(`${this.apiBaseURL}/fetch_section?page=`);
        },
        ...
        validate() {
            if (this.$refs.form.validate()) {
                ...
                this.insertOrUpdate(
                    `${this.apiBaseURL}/insert_section`,
        ...
        editData(id) {
            this.editOrFetch(
                `${this.apiBaseURL}/fetch_single_section/${id}`
            ).then(({ data }) => {
        ...
        clearP(id) {
            this.confirmMsg().then(({ res }) => {
                this.delGlobal(`${this.apiBaseURL}/delete_section/${id}`).then(
                    ({ data }) => {
